    ...
    <footer className="bg-[var(--ink)] text-white mt-auto">
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid md:grid-cols-4 gap-12 mb-12">
          ...
          {/* Navigation */}
          <div>
            <h4 className="text-xs font-semibold tracking-wider uppercase text-white/30 mb-4">Navigation</h4>
            <nav className="flex flex-col gap-2.5">
              {navigation.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="text-sm text-white/50 hover:text-[var(--sage)] transition-colors"
                >
                  {item.label}
                </a>
              ))}
          ...
          {/* Social */}
          <div>
            <h4 className="text-xs font-semibold tracking-wider uppercase text-white/30 mb-4">Connect</h4>
              ...
              {socialPlatforms.map((platform) => {
                ...
                return (
                  <a
                    ...
                    className="w-9 h-9 rounded-lg border border-white/10 bg-white/5 flex items-center justify-center hover:border-[var(--sage)]/30 hover:bg-[var(--sage)]/10 transition-all duration-300"
                    title={platform.name}
                  >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between pt-8 border-t border-white/10 gap-4">
          ...
          <a
            href="#home"
            className="flex items-center gap-2 text-xs text-white/30 hover:text-[var(--sage)] transition-colors"
          >
            Back to top
